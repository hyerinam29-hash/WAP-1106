...
**API 클라이언트:**
- `lib/api/tour-api.ts` - 한국관광공사 API 클라이언트 (7개 API 함수 구현 완료)
- `lib/api/supabase-api.ts` - Supabase 데이터베이스 API 클라이언트 (북마크 기능: 4개 함수 구현 완료)
...
**데이터베이스:**
- `supabase/migrations/myTrip_schema.sql` - users 테이블 생성 마이그레이션
- `supabase/migrations/20250108120000_create_bookmarks_table.sql` - bookmarks 테이블 생성 마이그레이션
